...
class Network {
 private:
  inline static const string MODEL_DIR = "../resource/model/";
 ...
 public:
  ...
  void train(const tensor_t &train_data,
    ...
    cout << "Training completed successfully!" << std::endl;
    cout << std::string(60, '=') << std::endl;
  }
  ...
  void save(const string &fileName) const {
    ...
    const string fullPath = MODEL_DIR + fileName;
    ...
    file << model.dump(2);
    file.close();
    cout << "Model saved as " << fileName << endl;
  }
 ...
 private:
  ...
  string _setTempDir(const steady_clock::time_point &tp) const {
    ...
    filesystem::create_directories(MODEL_DIR + dir);
    return dir;
  }
  ...
  void _getBatchInPlace(const tensor_t &src, size_t start, size_t batchSize, tensor_t &dest) const {
    const size_t featureSize = src.totalSize() / src.shape[0];

    // Direct memory copy for better performance
    const val_t *srcPtr = src.data.data() + (start * featureSize);
  ...
  void _setTrainingMode(bool training) {
    for (auto &layer : layers_) {
      if (auto bnLayer = dynamic_cast<BatchNormLayer *>(layer.get())) {
        bnLayer->setTraining(training);
      } else if (auto dropoutLayer = dynamic_cast<DropoutLayer *>(layer.get())) {
