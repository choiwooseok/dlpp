...
class Network {
 public:
  ...
  void addLayer(std::unique_ptr<BaseLayer> layer) { layers_.emplace_back(std::move(layer)); }

  // Forward prop
  Tensor forward(const Tensor &input) const {
    Tensor output = input;
    for_each(layers_.begin(),
             layers_.end(),
             [&](const auto &layer) {
  ...
  void train(
    ...
    for (auto epoch : views::iota(0, epochs)) {
      ...
      while (gen.next()) {
        ...
        // Progress output
        std::cout << std::format("\rEpoch {}/{} - lr {:.6f}, Batch {}/{}",
                                 epoch + 1, epochs,
      ...
      std::cout << std::format("Epoch {} - Loss: {:.6f} | Time: {}s\n",
                               epoch + 1,
                               totalLoss / static_cast<val_t>(numSamples),
                               elapsed.count());
    }
  ...
  class Result {
   public:
    struct ClassMetrics {
      ...
      double accuracy() const noexcept { return total > 0 ? static_cast<double>(correct) / total : 0.0; }
    };
 ...
 private:
  ...
  void _setOptimizer(Optimizer *optimizer) { optimizer_ = optimizer; }
