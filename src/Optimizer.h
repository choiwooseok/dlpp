...
class Optimizer {
 public:
  ...
  virtual double getLearningRate() const { return learningRate_; }
 ...
 public:
  ...
  void update(mat_t& param, const mat_t& grad, const std::string& paramId) override {
    ...
    if (nesterov_) {
      ...
      param.noalias() -= static_cast<val_t>(learningRate_) *
                         (static_cast<val_t>(momentum_) * v + effectiveGrad);
    } else {
  ...
  bool isNesterov() const { return nesterov_; }
  void setNesterov(bool nesterov) { nesterov_ = nesterov; }

  double getWeightDecay() const { return weightDecay_; }
  void setWeightDecay(double decay) { weightDecay_ = decay; }
 ...
 public:
  ...
  void update(mat_t& param, const mat_t& grad, const std::string& paramId) override {
    ...
    m = static_cast<val_t>(beta1_) * m +
        static_cast<val_t>(1.0 - beta1_) * effectiveGrad;

    // Update biased second raw moment estimate: v = beta2 * v + (1 - beta2) * grad^2
    v = static_cast<val_t>(beta2_) * v +
        static_cast<val_t>(1.0 - beta2_) * effectiveGrad.array().square().matrix();
    ...
    param.array() -= static_cast<val_t>(learningRate_) *
...
class CyclicLR : public Optimizer {
 public:
  ...
  explicit CyclicLR(double baseLR = 0.001,
                    double maxLR = 0.006,
                    size_t stepSize = 2000,
                    Policy policy = Policy::TRIANGULAR,
                    double gamma = 0.99994,
                    double momentum = 0.9,
                    bool nesterov = false,
                    double weightDecay = 0.0)
      : Optimizer(baseLR),
  ...
  void update(mat_t& param, const mat_t& grad, const std::string& paramId) override {
    ...
    if (nesterov_) {
      // Nesterov momentum
      param.noalias() -= static_cast<val_t>(currentLR_) *
                         (static_cast<val_t>(momentum_) * v + effectiveGrad);
    } else {
      // Standard momentum
  ...
  double getLearningRate() const override { return getCurrentLearningRate(); }

  // Getters
  double getCurrentLearningRate() const { return currentLR_; }
  double getBaseLR() const { return baseLR_; }
  double getMaxLR() const { return maxLR_; }
  size_t getStepSize() const { return stepSize_; }
  size_t getGlobalStep() const { return globalStep_; }
  size_t getCycleCount() const { return cycleCount_; }
  Policy getPolicy() const { return policy_; }

  // Setters
  void setBaseLR(double lr) { baseLR_ = lr; }
  void setMaxLR(double lr) { maxLR_ = lr; }
  void setStepSize(size_t size) { stepSize_ = size; }
...
class OneCycleLR : public Optimizer {
 public:
  explicit OneCycleLR(double maxLR = 0.1,
                      size_t totalSteps = 10000,
                      double pctStart = 0.3,
                      double divFactor = 25.0,
                      double finalDivFactor = 10000.0,
                      double momentum = 0.9,
                      double baseMomentum = 0.85,
                      double maxMomentum = 0.95,
                      bool nesterov = false,
                      double weightDecay = 0.0)
  ...
  void update(mat_t& param, const mat_t& grad, const std::string& paramId) override {
    ...
    if (nesterov_) {
      param.noalias() -= static_cast<val_t>(currentLR_) *
                         (static_cast<val_t>(currentMomentum_) * v + effectiveGrad);
    } else {
      param.noalias() -= static_cast<val_t>(currentLR_) * v;
    }
  }
  ...
  // Getters
  double getCurrentLearningRate() const { return currentLR_; }
  double getCurrentMomentum() const { return currentMomentum_; }
  size_t getGlobalStep() const { return globalStep_; }
