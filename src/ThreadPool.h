...
class ParallelUtil {
 public:
  static constexpr size_t THRESHOLD = 10000;

  template <typename BinaryOp>
  static val_t p_transform_reduce(const val_t* first1, const val_t* first2, size_t size, BinaryOp op, val_t init = val_t(0)) {
    if (size < THRESHOLD) {
  ...
  static void p_transform(const val_t* first, val_t* result, size_t size, UnaryOp op) {
    if (size < THRESHOLD) {
    ...
    for (size_t t = 0; t < numThreads; ++t) {
      ...
      futures.push_back(pool.enqueue([first, result, start, end, op]() {
        std::transform(first + start, first + end, result + start, op);
      }));
    }
