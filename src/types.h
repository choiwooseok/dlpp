...
class Tensor {
 ...
 public:
  // Dimension queries
  size_t strides(int idx) const { return strides_[idx]; }

  // number of dimensions
  size_t dim() const { return shape_.size(); }

  const std::vector<size_t> &shape() const { return shape_; }
  size_t shape(int idx) const { return shape_[idx]; }
  ...
  const vec_t &flatten() const { return data_; }
  ...
  // pointer access
  constexpr val_t *data() noexcept { return data_.data(); }
  constexpr const val_t *data() const noexcept { return data_.data(); }
  ...
  void reshape(const std::vector<size_t> &newShape) {
    size_t newSize = std::accumulate(newShape.begin(), newShape.end(), 1UL, std::multiplies<size_t>());
    assert(newSize == size() && "Reshape: new shape must have same total size");
