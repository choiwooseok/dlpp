...
class NetworkBuilder {
 public:
  ...
  NetworkBuilder &conv(int inChannels, int outChannels, int kernelHeight, int kernelWidth, int stride = 1, int pad = 0, INIT random = INIT::XAVIER) {
    network_.addLayer(std::make_unique<Conv2DLayer>(inChannels, outChannels, kernelHeight, kernelWidth, stride, pad, random));
    return *this;
  }
  ...
  // type : ["avg", "max"]
  NetworkBuilder &pool(const std::string &type, int channels, int poolWidth, int poolHeight, int stride = 1, int pad = 0) {
    if (type == "avg") {
  ...
  // type : ["avg", "max"]
  NetworkBuilder &pool(const std::string &type, BasePoolingLayer::WithInputDims tag, int inputWidth, int inputHeight, int channels, int poolWidth, int poolHeight, int stride = 1, int pad = 0) {
    if (type == "avg") {
      network_.addLayer(std::make_unique<AveragePoolingLayer>(tag, inputWidth, inputHeight, channels, poolWidth, poolHeight, stride, pad));
    }
    if (type == "max") {
      network_.addLayer(std::make_unique<MaxPoolingLayer>(tag, inputWidth, inputHeight, channels, poolWidth, poolHeight, stride, pad));
    }
    return *this;
  }
