...
class SoftmaxLayer : public BaseActivation {
 ...
 private:
  void _softmaxSequential(const Tensor &input, Tensor &output,
                          size_t batchSize, size_t numClasses) const {
    for (size_t n = 0; n < batchSize; ++n) {
      softmaxSample(input, output, n, numClasses);
    }
  }

  void _softmaxParallel(const Tensor &input, Tensor &output,
                        size_t batchSize, size_t numClasses) const {
    ...
    for (size_t n = 0; n < batchSize; ++n) {
      futures.push_back(pool.enqueue([this, &input, &output, n, numClasses]() {
        softmaxSample(input, output, n, numClasses);
      }));
    }
  ...
  void _backwardSequential(const Tensor &dY, const Tensor &softmaxOutput,
                           Tensor &dX, size_t batchSize, size_t numClasses) const {
    for (size_t n = 0; n < batchSize; ++n) {
      backwardSample(dY, softmaxOutput, dX, n, numClasses);
    }
  }

  void _backwardParallel(const Tensor &dY, const Tensor &softmaxOutput,
                         Tensor &dX, size_t batchSize, size_t numClasses) const {
    auto &pool = GlobalThreadPool::getInstance();
    std::vector<std::future<void>> futures;
    futures.reserve(batchSize);
    ...
    for (size_t n = 0; n < batchSize; ++n) {
      futures.push_back(pool.enqueue([this, &dY, &softmaxOutput, &dX, n, numClasses]() {
        backwardSample(dY, softmaxOutput, dX, n, numClasses);
      }));
    }
  ...
  void softmaxSample(const Tensor &input, Tensor &output, size_t sampleIdx,
    ...
    std::transform(inPtr, inPtr + numClasses, outPtr,
                   [maxVal](val_t v) { return std::exp(v - maxVal); });

    val_t sumExp = std::reduce(outPtr, outPtr + numClasses, val_t(0));

    // Normalize to get probabilities
    const val_t invSum = val_t(1) / sumExp;
    std::transform(outPtr, outPtr + numClasses, outPtr,
                   [invSum](val_t v) { return v * invSum; });
  }

  // Compute backward pass for a single sample
  void backwardSample(const Tensor &dY, const Tensor &softmaxOutput,
                      Tensor &dX, size_t sampleIdx, size_t numClasses) const {
    const size_t offset = sampleIdx * numClasses;
    ...
    // Compute dot product
    val_t dotProduct = std::transform_reduce(
        dYPtr, dYPtr + numClasses,
        softmaxPtr,
        val_t(0));

    // Compute gradient: dX_i = softmax_i * (dY_i - dotProduct)
    std::transform(dYPtr, dYPtr + numClasses, softmaxPtr, dXPtr,
                   [dotProduct](val_t dy, val_t sm) {
                     return sm * (dy - dotProduct);
                   });
  }
};