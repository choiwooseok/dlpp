...
class Conv2DLayer : public BaseLayer {
 public:
  ...
  Tensor forward(const Tensor &X) override {
    ...
    if (X.shape(1) != inChannels_) {
      throw LayerException(getName(), std::format("input channel size mismatch X.shape(1): {} inChannels_: {}", X.shape(1), inChannels_));
    }

    dimCache_ = ConvUtil::getDimensions(X, kernelHeight_, kernelWidth_, stride_, pad_);
    if (dimCache_.batchSize == 0) {
      return _createOutputTensor();
    }

    _ensureWorkspaceSize();

    if (enableParallel_ && dimCache_.batchSize >= 4) {
  ...
  Tensor backward(const Tensor &dY) override {
    assert(inputCache_.dim() == 4);

    if (dimCache_.batchSize == 0) {
      return Tensor(inputCache_.shape());
    }
  ...
  void info() override {
    std::cout << std::format("[inC={} outC={} k={}x{} s={} p={} parallel={}]",
                             inChannels_, outChannels_, kernelHeight_,
 ...
 private:
  Tensor _createOutputTensor() const {
    return Tensor({static_cast<size_t>(dimCache_.batchSize),
                   static_cast<size_t>(outChannels_),
                   static_cast<size_t>(dimCache_.outputHeight),
                   static_cast<size_t>(dimCache_.outputWidth)});
  }
  ...
  void _batchedIm2colParallel(const Tensor &X) {
    ...
    for (int n = 0; n < dimCache_.batchSize; ++n) {
      futures.push_back(pool.enqueue([&X, n, this]() {
        return ConvUtil::im2col(X, n, dimCache_);
      }));
    }
  ...
  Tensor _batchedCol2im() const {
    Tensor dX({static_cast<size_t>(dimCache_.batchSize),
               static_cast<size_t>(dimCache_.channels),
               static_cast<size_t>(dimCache_.inputHeight),
               static_cast<size_t>(dimCache_.inputWidth)});
    dX.fill(0);
