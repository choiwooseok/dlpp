...
class BasePoolingLayer : public BaseLayer {
 public:
  ...
  explicit BasePoolingLayer(const std::string &name,
                            int channels,
                            int poolWidth,
                            int poolHeight,
                            int stride,
  ...
  BasePoolingLayer(const std::string &name,
                   int inputWidth,
                   int inputHeight,
                   int channels,
                   int poolWidth,
    ...
    std::cout << std::format("{} initialized with input dims: {}x{}, output dims: {}x{}", getName(), inputHeight_, inputWidth_, outputHeight_, outputWidth_) << std::endl;
  }
 ...
 public:
  ...
  void info() override {
    ...
    if (!autoDetect) {
      std::cout << std::format("[in={}x{} ch={} pool={}x{} s={} p={} out={}x{}]", inputHeight_, inputWidth_, channels_, poolHeight_, poolWidth_, stride_, pad_, outputHeight_, outputWidth_);
    } else {
      std::cout << std::format("[in=auto ch={} pool={}x{} s={} p={} out=auto]", channels_, poolHeight_, poolWidth_, stride_, pad_);
    }
  }
