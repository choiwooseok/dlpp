...
class MeanSquaredError {
 public:
  static val_t f(const Tensor& label, const Tensor& predicted) {
    ...
    val_t loss = ParallelUtil::p_transform_reduce(labelData, predData, size,
                                                  [](val_t l, val_t p) {
                                                    val_t diff = l - p;
                                                    return diff * diff;
                                                  });
  ...
  static Tensor df(const Tensor& label, const Tensor& predicted) {
    ...
    ParallelUtil::p_transform(predicted.data(),
                              label.data(),
                              grad.data(),
                              size,
                              [factor](val_t p, val_t l) {
                                return factor * (p - l);
                              });
...
class RootMeanSquaredError {
 public:
  static val_t f(const Tensor& label, const Tensor& predicted) {
    ...
    val_t mse = ParallelUtil::p_transform_reduce(labelData, predData, size,
                                                 [](val_t l, val_t p) {
                                                   val_t diff = l - p;
                                                   return diff * diff;
                                                 });
...
class CategoricalCrossEntropy {
 ...
 public:
  static val_t f(const Tensor& label, const Tensor& predicted) {
    ...
    val_t totalLoss = ParallelUtil::p_transform_reduce(labelData, predData, totalSize,
                                                       [](val_t y, val_t p_raw) {
                                                         val_t p = std::clamp(p_raw, EPSILON, val_t(1) - EPSILON);
                                                         return y * std::log(p);
                                                       });
  ...
  static Tensor df(const Tensor& label, const Tensor& predicted) {
    ...
    ParallelUtil::p_transform(predicted.data(), label.data(), grad.data(), totalSize,
                              [factor](val_t p_raw, val_t y) {
                                val_t p = std::clamp(p_raw, EPSILON, val_t(1) - EPSILON);
                                return -factor * y / p;
                              });
