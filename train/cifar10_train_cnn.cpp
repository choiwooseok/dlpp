...
int main(int argc, char** argv) {
  ...
  Network nn;
  nn.load("cifar10_model_cnn_0.67.json");
  ...
  Tensor train_data = Tensor::fromMat(toEigenMatrix(train_ds.getPixels()));
  ...
  Tensor test_data = Tensor::fromMat(toEigenMatrix(test_ds.getPixels()));
  Tensor test_label = Tensor::fromMat(toEigenMatrix(test_ds.getLabels()));

  auto reshape = [](Tensor& tensor) {
    tensor.reshape({tensor.shape(0),
  ...
  double max_acc = 0.0;
  nn.train<CCE>(train_data, train_label,
                epochs,
                &opt,
                batchSize,
                [&]() {
                  auto ret = nn.test<Cifar10Data>(test_data, test_label, [&](int idx, int pred) {
                    // std::cout << std::format("\npredicted: {} ({})\n",
                    //                          Cifar10Data::classToString(static_cast<Cifar10Data::Class>(pred)),
                    //                          pred);
                  });
