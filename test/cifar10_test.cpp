...
class CIFAR10Fixture : public ::testing::TestWithParam<std::string> {
 protected:
  ...
  Cifar10Data data = Cifar10DataBuilder()
                         .withTestData()
                         .build();
  Tensor test_data = Tensor::fromMat(toEigenMatrix(data.getPixels()));
...
TEST_P(CIFAR10Fixture, CIFAR10) {
  auto ret = nn.test<Cifar10Data>(
      test_data, test_label,
      [&](int idx, int predicted) {
        std::cout << std::format("\npredicted: {} ({})\n",
...
INSTANTIATE_TEST_SUITE_P(
    CIFAR10,
    CIFAR10Fixture,
    ::testing::Values("cifar10_model_cnn_0.68.json"));
